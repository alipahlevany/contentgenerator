...
migrations = Path("contents/migrations")

files = sorted(migrations.glob("*_seed_generation_v2.py"))

if not files:
    raise SystemExit(
        "Run:\npython manage.py makemigrations contents --empty -n seed_generation_v2"
    )
...
print(f"OK: Seed migration written to {migration}")
