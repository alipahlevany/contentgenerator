...
def validate_callback_url(url):
    ...
    try:
        ...
        raise ValidationError("Callback URL hostname could not be resolved.") from exc

    for address in addresses:
        ip = ipaddress.ip_address(address[4][0])
        if not ip.is_global:
            raise ValidationError("Callback URL resolves to a blocked network.")
...
def deliver_content(delivery_id):
    ...
    try:
        ...
        _mark_failed(delivery.pk, "Temporary callback connection failure.")
        raise RetryableDeliveryError("Temporary callback connection failure.") from exc
    except requests.RequestException:
        _mark_failed(delivery.pk, "Callback request failed.")
        return ContentDelivery.objects.get(pk=delivery.pk)

    if 200 <= response.status_code < 300:
        with transaction.atomic():
            delivery = ContentDelivery.objects.select_for_update().get(pk=delivery.pk)
            delivery.status = "success"
            delivery.delivered_at = timezone.now()
            delivery.last_error = ""
            delivery.save(
                update_fields=["status", "delivered_at", "last_error", "updated_at"]
            )
        return delivery

    message = f"Callback returned HTTP {response.status_code}."
    _mark_failed(delivery.pk, message)
...
def _mark_failed(delivery_id, message):
    ContentDelivery.objects.filter(pk=delivery_id).update(
        status="failed",
        last_error=message,
